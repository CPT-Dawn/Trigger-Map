...
function coerceStressLevel(
  value: StressLevelValue | null | undefined,
): StressLevel {
  if (
    value === "none" ||
    value === "low" ||
...
function normalizeStressLevel(
  value: StressLevelValue | string | null | undefined,
): StressLevel {
  if (
    value === "none" ||
    value === "low" ||
...
function formatSnapshotQuantityAndUnit(
...
function sortMasterItems(items: UserItemRow[]) {
  return [...items].sort((left, right) =>
    getMasterItemLabel(left).localeCompare(
      getMasterItemLabel(right),
      undefined,
      { sensitivity: "base" },
    ),
  );
...
function buildMasterItemSearchText(item: UserItemRow) {
  ...
  return [item.display_name, item.name, quantityAndUnit]
...
const ExpandableLogCard = React.memo(function ExpandableLogCard({
  ...
  const cardMotionStyle = useAnimatedStyle(() => {
    ...
    return {
      transform: [
        { translateY: -lift },
        { scale: 1 - pressProgress.value * 0.008 },
      ] as const,
    };
  });
    ...
    >
      ...
      >
        ...
        >
          <View style={styles.entryTopRow}>
            <View style={styles.entryLeading}>
              <View
                style={[styles.iconWrap, { backgroundColor: config.container }]}
              >
        ...
        {isExpanded ? (
          ...
          >
            <Reanimated.View
              entering={FadeInDown.duration(160).delay(10)}
              style={styles.entryActionSlot}
            >
              <Pressable
                accessibilityRole="button"
            ...
            <Reanimated.View
              entering={FadeInDown.duration(160).delay(35)}
              style={styles.entryActionSlot}
            >
              <Pressable
                accessibilityRole="button"
...
export default function LogsScreen() {
  ...
  const { toast, toastKey } = useLocalSearchParams<{
    toast?: string;
    toastKey?: string;
  }>();
  ...
  const [editSelectedItem, setEditSelectedItem] = useState<SelectedItem | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    const nextToastKey =
      typeof toastKey === "string" && toastKey.length > 0
        ? toastKey
        : nextMessage;
  ...
  const filteredEditItemOptions = useMemo(() => {
    ...
    return editItemOptions.filter((item) =>
      buildMasterItemSearchText(item).includes(normalizedEditItemSearch),
    );
  ...
  const insertPayloadRow = async (entry: TimelineEntry) => {
    ...
    if (entry.payload.kind === "medicine") {
      const itemDisplayName =
        entry.payload.row.item_display_name?.trim() || entry.title;
      const payload = {
        id: entry.payload.row.id,
        user_id: user.id,
  ...
  const deleteLogEntry = async (entry: TimelineEntry) => {
    ...
    try {
      runLocalTransaction(() => {
        if (entry.payload.kind === "pain") {
          db.runSync("DELETE FROM pain_logs WHERE id = ? AND user_id = ?;", [
            entry.payload.row.id,
            user.id,
          ]);
          addToSyncQueue(
            "pain_logs",
            "DELETE",
            { id: entry.payload.row.id, user_id: user.id },
            { userId: user.id },
          );
        } else if (entry.payload.kind === "stress") {
          db.runSync("DELETE FROM stress_logs WHERE id = ? AND user_id = ?;", [
            entry.payload.row.id,
            user.id,
          ]);
          addToSyncQueue(
          ...
          db.runSync("DELETE FROM food_logs WHERE id = ? AND user_id = ?;", [
            entry.payload.row.id,
            user.id,
          ]);
          addToSyncQueue(
    ...
    async (mode: "initial" | "refresh" = "refresh") => {
      ...
      try {
        ...
        const medicineItems = new Map<string, UserItemRow>(
          medicineItemRows.map((item) => [item.id, item]),
        );
        const foodItems = new Map<string, UserItemRow>(
          foodItemRows.map((item) => [item.id, item]),
        );
        ...
        const stressEntries: TimelineEntry[] = stressRows.map(
          (row: StressLogRow) => ({
            id: `stress-${row.id}`,
            type: "stress",
            logDate: row.log_date,
        ...
        const medicineEntries: TimelineEntry[] = medicineRows.map(
          (row: MedicineLogRow) => {
            const medicineItem = medicineItems.get(row.medicine_id) ?? null;
            const snapshotTitle =
              row.item_name?.trim() || row.item_display_name?.trim() || "";
            const snapshotDose = formatSnapshotQuantityAndUnit(
              row.item_quantity,
              row.item_unit,
            );
            const medicineDose =
              snapshotDose || formatItemQuantityAndUnit(medicineItem);
            const resolvedTitle =
              snapshotTitle || buildDisplayName(medicineItem);
            ...
            return {
              ...
              loggedAt: row.logged_at,
              title: resolvedTitle,
              subtitle: medicineDose
                ? `${medicineDose} • ${formatTime(row.logged_at)}`
        ...
        const foodEntries: TimelineEntry[] = foodRows.map((row: FoodLogRow) => {
          ...
          const snapshotTitle =
            row.item_name?.trim() || row.item_display_name?.trim() || "";
          const snapshotServing = formatSnapshotQuantityAndUnit(
            row.item_quantity,
            row.item_unit,
          );
          const serving =
            snapshotServing || formatItemQuantityAndUnit(foodItem);
          ...
          return {
            ...
            title: resolvedTitle,
            subtitle: serving
              ? `${serving} • ${formatTime(row.logged_at)}`
              : formatTime(row.logged_at),
  ...
  useEffect(() => {
    ...
    const nextMode: "initial" | "refresh" = hasFocusedLoadRef.current
      ? "refresh"
      : "initial";
  ...
  const visibleSections = useMemo(
    () => groupEntriesByDate(visibleEntries),
    [visibleEntries],
  );
  ...
  const pendingDeleteConfig = pendingDeleteEntry
    ? typeConfig[pendingDeleteEntry.type]
    : null;
  const deleteModalScrimColor =
    appliedTheme === "dark" ? colors.background : colors.text;
        ...
        >
          {filterOptions.map((option) => {
            ...
            const categoryConfig =
              option.value === "all" ? null : typeConfig[option.value];
            const selectedBackground =
              categoryConfig?.container ?? colors.surfaceContainerHighest;
            ...
            return (
              <Chip
                ...
                textStyle={{
                  color: selected ? selectedTextColor : colors.textMuted,
                }}
              >
                {option.label}
              </Chip>
  ...
  const renderEmptyState = () => {
    ...
    const activeFilter = isFiltered
      ...
      : null;
    const activeFilterConfig = isFiltered
      ? typeConfig[filter as LogType]
      : null;
    const emptyIcon = isFiltered
      ? (activeFilter?.icon ?? "filter-off")
      : "clipboard-text-outline";
    const emptyIconContainer =
      activeFilterConfig?.container ?? colors.primaryContainer;
    const emptyAccentColor =
      activeFilterConfig?.iconColor ?? colors.onPrimaryContainer;
        ...
        >
          <View style={styles.emptyTopRow}>
            <View
              style={[
                styles.emptyIconWrap,
                { backgroundColor: emptyIconContainer },
              ]}
            >
              <MaterialCommunityIcons
                name={emptyIcon}
            ...
            <View style={styles.emptyCopyBlock}>
              ...
              >
                {isFiltered
                  ? `${activeFilter?.label ?? "Selected"} not logged yet!`
                  : "No logs yet!"}
              </Text>
              <Text
                variant="bodyMedium"
        ...
        <View style={styles.modalContainer}>
          <Pressable
            style={[
              ...
              { backgroundColor: colors.text, opacity: 0.45 },
            ]}
            onPress={closeEditor}
          />
          <KeyboardAvoidingView
          ...
          >
            <Reanimated.View
              entering={FadeInDown.duration(170).easing(
                    ...
                    <>
                      ...
                      >
                        ...
                        >
                          ...
                          <Switch
                            value={editPainSwelling}
                            onValueChange={setEditPainSwelling}
                            color={colors.onErrorContainer}
                          />
                        </View>
                      </View>
                    ...
                    <View
                      style={[
                        styles.editSectionCard,
                        {
                          ...
                          borderColor: colors.ghostBorder,
                          borderLeftColor:
                            editingConfig?.iconColor ?? colors.primary,
                        },
                      ]}
                    >
                      <Text
        ...
        <View style={styles.modalContainer}>
          ...
          >
            <Pressable
              style={[
                styles.modalBackdrop,
                { backgroundColor: deleteModalScrimColor, opacity: 0.66 },
              ]}
          ...
          >
            ...
            >
              <AppCard
                variant="solid"
                style={[styles.modalCard, styles.deleteModalCard]}
              >
                <View style={styles.deleteModalTopRow}>
                  <View style={styles.deleteModalHeadingBlock}>
                    ...
                    <Text
                      variant="titleMedium"
                      style={[styles.deleteModalTitle, { color: colors.text }]}
                      numberOfLines={1}
                    >
                      Confirm Delete
                    </Text>
                ...
                >
                  <View style={styles.deleteModalSummaryTop}>
                    ...
                    >
                      <MaterialCommunityIcons
                        name={
                          pendingDeleteConfig?.icon ?? "clipboard-text-outline"
                        }
                        size={18}
                        color={
                          pendingDeleteConfig?.iconColor ?? colors.textMuted
                        }
                      />
                    </View>
                    ...
                    <Chip
                      ...
                      textStyle={{
                        color: pendingDeleteConfig?.iconColor ?? colors.text,
                      }}
                    >
                      {pendingDeleteEntry
                        ? titleCase(pendingDeleteEntry.type)
                        : "Entry"}
                    </Chip>
                  </View>
                </View>
